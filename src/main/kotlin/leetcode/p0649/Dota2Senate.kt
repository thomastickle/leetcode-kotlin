 ...
 * 1. **Ban one senator's right**: A senator can make another senator lose all his rights in this
 *    and all the following rounds.
 * 2. **Announce the victory**: If this senator found the senators who still have rights to vote
 ...
 * `'D'` represent the Radiant party and the Dire party. Then if there are `n` senators, the size
 * of the given string will be `n`.
 *
 ...
 *   And in the round 2, the first senator can just announce the victory since he is the only guy in
 ...
 *   And in the round 2, the third senator can just announce the victory since he is the only guy in
 *   the senate who can vote.
 *
 * Constraints:
 * - `n == senate.length`
...
class Solution {
    fun predictPartyVictory(senate: String): String {
        ...
        while (radiant.isNotEmpty() && dire.isNotEmpty()) {
            val radiantTurn  = radiant.removeFirst()
            val direTurn = dire.removeFirst()
            if (radiantTurn < direTurn) {
                radiant.addLast(radiantTurn + senate.length)
            } else {
                dire.addLast(direTurn + senate.length)
            }
        }
