 ...
 *   milliseconds (including the new request). Specifically, return the number of requests that
...
class RecentCounter {
    ...
    private val calls = ArrayDeque<Int>()

    fun ping(t: Int): Int {
        calls.addLast(t)
        val oldestPossibleEvent = t - 3000
        while (calls.first() < oldestPossibleEvent) {
            calls.removeFirst()
        }
        return calls.size
    }
