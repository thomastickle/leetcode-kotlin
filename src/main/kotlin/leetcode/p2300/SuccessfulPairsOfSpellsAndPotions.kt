 ...
 * respectively, where `spells[i]` represents the strength of the `i`th spell and `potions[j]`
...
class Solution {
    fun successfulPairs(spells: IntArray, potions: IntArray, success: Long): IntArray {
        fun lowerBound(potions: IntArray, minimumStrength: Long): Int {
            var low = 0
            var high = potions.size

            while (low < high) {
                val mid = low + (high - low) / 2
                if (potions[mid] < minimumStrength) {
                    low = mid + 1
                } else {
                    high = mid
                }
        ...
        for (index in spells.indices) {
            val minimumStrength = (success - 1) / spells[index] + 1
            val firstSuccessful = lowerBound(potions, minimumStrength)
            result[index] = potions.size - firstSuccessful
        }
